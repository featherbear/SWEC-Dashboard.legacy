...
class User(UserModel):
    def __init__(self, id):
        ...
        if self.id == 0:
            ...
            data = Database.fetchOne("SELECT username, userType, firstName, lastName FROM users WHERE id = ?", (id,))
            if not data: raise NoUserException()
...
class UserSession(User):
    ...
    def __init__(self, _type, _username, _expiry):
        self.type, self.username, self.expiry = map(lambda o: o.decode() if type(o) == bytes else o, [_type, _username, _expiry])
        ...
        if self.type == AUTH_TYPE.AUTH_USER and self.username == config["SUPERUSER"]["username"]:
            ...
            self.id, self.firstName, self.lastName = Database.fetchOne("SELECT id, firstName, lastName FROM users WHERE userType = ? AND username = ?", (self.type, self.username))
