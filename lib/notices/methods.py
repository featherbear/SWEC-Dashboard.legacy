...
def submitNotice(title, description, author, date, endDate=None, priority=0, active=1) -> Notice:
    ...
    notice = Notice(title, description, author, date, endDate, priority, time, isApproved, active)
    notice.id = entry
    return notice
...
def editNotice(id: int, data: dict):
    ...
    return Database.update(
        ...
        WHERE
          id = ?
        """.format(", ".join(map(lambda k: k + " = ?", data.keys())))
        ,(*data.values(), id,)
    )
...
def fetch(start: int, number: int, uid=None, showPending=False, showInactive=False):
    WHERE = []
    if not showPending:
        WHERE.append("(notices.approved = 1 OR notices.author = {})".format(uid))
    if not showInactive:
